...
class Product
{
    ...
    /**
     * Возвращаем товары указанной категории EXCEL
    ...
    public static function getCategoryList($categoryId)
    {
        ...
        // Текст запроса к БД
        $sql = 'SELECT name, price FROM product WHERE category_id = :category_id';
    ...
    public static function updateProductById($id, $options)
    {
        ...
        $result->bindParam(':description_seo', $options['description_seo'], PDO::PARAM_STR);
        $result->bindParam(':is_new', $options['is_new'], PDO::PARAM_INT);
        $result->bindParam(':is_recommended', $options['is_recommended'], PDO::PARAM_INT);
        $result->bindParam(':status', $options['status'], PDO::PARAM_INT);
        return $result->execute();
    }
    public static function createImages($id, $status = 0)
    {
        // Соединение с БД
        $db = Db::getConnection();
        ...
        $idd .= $id.$rand = rand(0, 999);
                // Если загружалось, переместим его в нужную папке, дадим новое имя
        $name_img = move_uploaded_file($_FILES["image"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . "/upload/images/products/{$idd}.jpg");
                // Получение и возврат результатов. Используется подготовленный запрос
        $result = $db->prepare($sql);
    ...
    public static function createProduct($options)
    {
        ...
        $sql = 'INSERT INTO product '
                ...
                . 'description, description_seo, title, is_new, is_recommended, status)'
                . 'VALUES '
                . '(:name, :code, :price, :category_id, :brand, :availability,'
                . ':description, :description_seo, :title, :is_new, :is_recommended, :status)';
    ...
    public static function getImageProduct($id)
    {
        ...
        {
            $resImg['name_img'] = $row['name_img'];
            $i++;
        }
        // Путь к изображению товара
        $pathToProductImage = $path . $resImg['name_img'] . '.jpg';
